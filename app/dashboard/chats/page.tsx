...
export default function ChatsPage() {
  ...
  useEffect(() => {
    ...
    fetchConversations().finally(() => setIsLoadingConvs(false));
  ...
  useEffect(() => {
    ...
    fetchMessages(activeConvId).finally(() => setIsLoadingMsgs(false));
  ...
  return (
    <HomePage active="chats">
      <div className="relative flex h-full flex-1 flex-col overflow-hidden bg-[#11101a]/90 lg:flex-row">
          ...
          <div className="flex-1 space-y-2 overflow-y-auto pr-1">
            {isLoadingConvs ? (
              ...
              <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
                ...
                <p className="text-xs text-white/20">Start one below.</p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        <p className="truncate text-sm font-medium text-white">{other.username}</p>
          ...
          {showNewConv ? (
            <div className="mt-3 space-y-2">
              <input
                ...
                autoFocus
                value={newUsername}
                onChange={(e) => { setNewUsername(e.target.value); setNewConvError(""); }}
              ...
              />
              {newConvError && <p className="text-xs text-red-400 px-1">{newConvError}</p>}
              <div className="flex gap-2">
                <button
                  onClick={handleNewConversation}
                  disabled={newConvLoading}
                  className="flex-1 rounded-xl bg-gradient-to-br from-[#b06cff] to-[#7d3cff] py-2 text-sm font-medium text-white transition-all hover:brightness-110 active:scale-95 disabled:opacity-60"
                >
                  {newConvLoading ? "Finding..." : "Start Chat"}
                </button>
                <button
                  onClick={() => { setShowNewConv(false); setNewUsername(""); setNewConvError(""); }}
                  className="rounded-xl border border-white/10 px-3 py-2 text-sm text-white/50 hover:text-white/80 transition-colors"
                >
                  Cancel
                </button>
              </div>
        ...
        <section className="relative z-10 flex flex-1 flex-col overflow-hidden border border-white/12 shadow-[inset_0_1px_0_rgba(255,255,255,0.12),0_30px_90px_rgba(0,0,0,0.45)] backdrop-blur-3xl m-3 rounded-[2.2rem] bg-white/[0.015]">
          ...
          {!activeConvId ? (
            <div className="relative z-10 flex flex-1 flex-col items-center justify-center gap-4 text-center px-8">
              <div className="text-6xl opacity-20">👻</div>
              <p className="text-lg font-medium text-white/30">Select a conversation</p>
              <p className="text-sm text-white/20">or start a new one to begin messaging anonymously</p>
            </div>
          ) : (
            <>
              {/* Header */}
              <header className="relative z-10 flex items-center justify-between gap-4 border-b border-white/10 px-6 py-4 bg-white/[0.02]">
              ...
              <div className="relative z-10 flex-1 overflow-y-auto p-6 space-y-4">
                {isLoadingMsgs ? (
                  <div className="flex flex-col gap-4 pt-4">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className={`flex items-end gap-3 ${i % 2 === 0 ? "justify-end" : ""} animate-pulse`}>
                  ...
                  <>
                    <div className="mx-auto w-fit rounded-full border border-white/8 bg-white/[0.04] px-3 py-1 text-[11px] text-white/45">
                      Channel Instantiated — {activeConv ? getOtherParticipant(activeConv).username : ""}
                    </div>
                      ...
                      return (
                        <div key={msg._id} className="animate-bubble-3d">
                          ...
                          <div className={`flex items-end gap-3 ${isOwn ? "justify-end" : "justify-start"}`}>
                            ...
                            <div className={`max-w-[75%] rounded-[1.4rem] border px-4 py-2.5 shadow-lg transition-all duration-300 ${
                              isOwn
              ...
              {/* Compose */}
              <footer className="relative z-10 p-4 border-t border-white/10 bg-white/[0.01]">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                  ...
                  <button
                    ...
                    className="rounded-xl bg-gradient-to-br from-[#b06cff] to-[#7d3cff] px-5 py-2.5 text-sm font-medium text-white shadow-md hover:brightness-110 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isSending ? "..." : "Send"}
                  </button>
                </div>
